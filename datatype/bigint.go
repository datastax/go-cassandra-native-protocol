...
var Bigint PrimitiveType = &primitiveType{code: primitive.DataTypeCodeBigint}

const lengthOfBigint = 8
	...
	if value == nil {
		...
		case uint:
			if uint64(v) > math.MaxInt64 {
				return nil, fmt.Errorf("cannot marshal bigint: value out of range: %v", value)
			}
			val = int64(v)
		case int64:
			val = v
		...
		case string:
			...
			if err != nil {
				return nil, fmt.Errorf("cannot marshal bigint: invalid string: %v", value)
			}
		default:
			return nil, fmt.Errorf("cannot marshal bigint: incompatible value: %v", value)
		}
		encoded = make([]byte, lengthOfBigint)
